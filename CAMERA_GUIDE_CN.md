...
5. 在人工清场和一次性授权下，按停止条件执行多步运动；每步再拍照并更新同一张地图。

相机已经安装在机械臂末端，手眼标定和真实多步闭环已经完成。尚未完成的是：学习式草莓
分割、环境碰撞模型以及双臂协同。大空间实验现在最多允许 8 步，但会按 coverage 目标、
平台期、候选可达性和输入健康提前停止。
...
Gemini 2 XL 不是由电脑 USB 单独供电。正确连接关系是：

```text
墙上插座 → 原装直流电源适配器 → OBox 的 DC 12V
电脑 USB → OBox 的 “to Host”
相机本体 → OBox 的 “to Device”
```

Orbbec 官方要求 OBox 的直流电源为 `12V、至少 2A`。电压必须正好是 12V；电流容量可以
是 2A 或更高，但插头尺寸和正负极必须与原装适配器完全一致。不能使用只标 `12V 1A` 的
适配器：它的容量不足，可能在设备启动或深度计算的瞬时负载下导致 OBox 重启、USB 掉线。

2026-09-13 的长闭环在第三个约 10 cm 动作后出现相机掉线；随后发现现场误用了
`12V 1A` 适配器，并在重新插线时触发电脑 USB 端口过流保护。因此在取得原装或经确认匹配的
`12V ≥2A` 电源以前，不允许再给该相机链路上电，也不能把问题归咎于 ROS 或 NBV。

每次上电前确认：

1. 适配器标签完整写有 `OUTPUT: 12V` 和至少 `2A`（或至少 `2000mA`）；
2. 使用原装适配器，或由懂电气的人确认插头尺寸、中心正负极完全匹配；
3. 相机接 `to Device`，电脑接 `to Host`，不能接反；
4. 线缆没有夹伤、破皮、焦味、变色或松动；
5. 若系统再次出现 `over-current condition`，立即拔下相机 USB 和电源，不换端口反复尝试。
...
后续会评估无需自行训练或只需少量标注的草莓分割模型，NBV 后半段无需重写。当前不依赖
需要额外申请权限的 SAM3。
