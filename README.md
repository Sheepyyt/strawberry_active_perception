...
- 独立 5–10 cm 配置已完成首次真机实验：相机实际移动 `48.95 mm / 2.78°`，同一张地图的
  ...
  建议在当时关节姿态下不可达，系统安全停止；这证明了厘米级单步链路，同时也明确了下一
  个软件任务是“在首选 NBV 不可达时，比较其他高收益且可达的备用方向”。
...
相机位姿是连续量，数学上有无限多个点，因此不能真的先列出“所有可达点”。下一版采用
有限而可重复的近似：

1. 在当前相机周围的 5 cm 和 10 cm 球壳上，按固定角度生成一批候选位置；每个候选都朝向
   当前锁定的同一个草莓目标。
2. Placo 从当前关节角逐个求 IK，先删除不可达、接近关节限位、过于奇异或关节变化过大的
   候选。
3. Gradient-NBV 使用当前这张体素地图给剩余候选逐个计算信息收益，而不是只给一个梯度
   方向。
4. 执行“收益最高且可达”的候选；到新位置后融合新的五帧观测，继续使用同一张地图。
5. 没有可达且有正收益的候选、coverage 达标或进入平台期时才正常停止。

这相当于先问机械臂“你能去哪儿”，再在它能去的地方里问 NBV“去哪儿最值得”，能够避免
当前这种“数学上的最好方向存在，但机械臂到不了”的情况。首版会保存所有候选的位置、IK
结果、关节余量和收益排序，便于在可视化中直接看到哪些点被接受、哪些点被拒绝。
...
并完成了一次真实 5 cm Gradient-NBV 运动。下一步不是继续放宽关节限位，而是给 NBV 与
Placo 之间增加可达备用视角：首选梯度方向不可达时，在保持正观察收益的候选中选择更接近
机械臂舒适区的方向，然后重新做最多三步的厘米级实验。与此同时，同一批相机图片会离线
比较 HSV 与学习式草莓分割；学习模型通过人工叠加图检查前不会控制机械臂。双臂、采摘和
无人值守连续运动暂不进入本阶段。
