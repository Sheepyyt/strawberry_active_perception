...
### 下一步：只在可达候选里选 NBV，并自适应运动幅度

相机位姿是连续量，数学上有无限多个点，因此不能真的先列出“所有可达点”。下一版采用
有限而可重复的近似：

1. 在当前相机周围按固定角度生成一批候选位置。距离由大到小覆盖约
   `10 / 7.5 / 5 / 2.5 / 1 cm`，必要时再回退到毫米级；每个候选都朝向当前锁定的同一个
   草莓目标。这里的 5 cm 不再是硬下限。
2. Placo 从当前关节角逐个求 IK，先删除不可达、接近关节限位、过于奇异或关节变化过大的
   候选。
3. Gradient-NBV 使用当前这张体素地图给剩余候选逐个计算信息收益，而不是只给一个梯度
   方向。候选必须比当前视角有正收益。
4. 先保留接近最高收益的一组候选，再优先选择运动更明显、离关节限位更远的候选。这样既
...
可重复、可计算的近似。首版会保存所有候选的位置、距离、IK、关节余量和收益排序，便于在
可视化中直接看到哪些点被接受、哪些点被拒绝。
...
Placo 之间增加“多方向、多距离”的可达候选：优先做 5–10 cm 的明显运动，大动作不可达就
换方向或逐级缩到 2.5/1 cm，仍有正收益时才运动。随后重新运行同一地图的连续闭环，直到
