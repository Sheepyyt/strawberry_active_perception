...
- 优先选择 5–10 cm 的相机动作，最多 8 步；若较大候选不可达，会换方向并依次回退到
  2.5/1/0.5 cm，而不是直接停止；真正停止死区仍是 1 mm；
- 累计平移不超过 60 cm、累计旋转不超过 90°；
- 默认 coverage 达到 20%，或连续 2 步新增都小于 0.5 个百分点时提前停止；如果候选
  网格里已经没有正收益且可达的点，也按“观察完成/受可达性约束收敛”正常停止而不是报错；
